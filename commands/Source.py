		...
		if message.messagePartsLength > 0:
			for commandName, command in GlobalStore.commandhandler.commands.iteritems():
					if commandName == message.messageParts[0] or (command.triggers and message.messageParts[0] in command.triggers):
						...
						return message.reply("You want to peek behind the magic of the {0} command? Sure, that's what open-source means, here you go: https://github.com/Didero/DideRobot/blob/master/commands/{0}.py".format(commandName), "say")
			else:
				return message.reply("Hmm, I don't have a module named '{0}'. Either you made a typo, or you should convince my owner(s) to create the '{0}' module so I can link it when it's done".format(message.messageParts[0]), "say")
