...
_PARSE_ISO_DURATION_REGEX = re.compile(r"P(?:(?P<year>\d+)Y)?(?:(?P<month>\d+)M)?(?:(?P<week>\d+)W)?(?:(?P<day>\d+)D)?T?(?:(?P<hour>\d+)H)?(?:(?P<minute>\d+)M)?(?:(?P<second>\d+)S)?")
...
def parseIsoDuration(isoString, formatstring=""):
	...
	result = _PARSE_ISO_DURATION_REGEX.search(isoString)
	if result is None:
